
<div class="fullpage">

	<!-- START CONTENT -->
	<div class="right_area">
		<!-- START COLORS -->
		<section id="colors" data-scroll-index="1">
			<!-- START PRIMARY -->
			<h3 class="slate">Primary Colors</h3>
			<p class="gray">These are first colors to be considered for a design. These primary colors reflect what should be used for text, links and lighter backgrounds.</p>
		  <div class="section primary flex">
		    <figure class="primary_color slate">
		      <div class="color"></div>
		      <h6 class="slate name">Slate</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="454E5A">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="69 78 90">
		      </p>
		    </figure>
		    <figure class="primary_color blue">
		      <div class="color"></div>
		      <h6 class="slate name">Blue</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="45A0F8">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="69 160 248">
		      </p>
		    </figure>
		    <figure class="primary_color gray">
		      <div class="color"></div>
		      <h6 class="slate name">Gray</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="8D989E">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="141 152 158">
		      </p>
		    </figure>
		    <figure class="primary_color light_gray">
		      <div class="color"></div>
		      <h6 class="slate name">Light Gray</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="247 250 252">
		      </p>
		    </figure>
		  </div>
		  <!-- END PRIMARY -->
		  <!-- START SECONDARY -->
			<h3 class="slate">Secondary Colors</h3>
			<p class="gray">We have defined a core set of secondary colors for you to start with. These are to be used in the supporting illustrations throughout the site and in combination with the primary neutral colors.</p>
		  <div class="section secondary flex">
		    <figure class="secondary_color purple">
		      <div class="color"></div>
		      <h6 class="slate name">Purple</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="454E5A">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="69 78 90">
		      </p>
		    </figure>
		    <figure class="secondary_color medium_blue">
		      <div class="color"></div>
		      <h6 class="slate name">Medium Blue</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="45A0F8">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="69 160 248">
		      </p>
		    </figure>
		    <figure class="secondary_color sage_green">
		      <div class="color"></div>
		      <h6 class="slate name">Sage Green</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="8D989E">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="141 152 158">
		      </p>
		    </figure>
		    <figure class="secondary_color red">
		      <div class="color"></div>
		      <h6 class="slate name">Red</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="247 250 252">
		      </p>
		    </figure>
		    <figure class="secondary_color orange">
		      <div class="color"></div>
		      <h6 class="slate name">Orange</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="247 250 252">
		      </p>
		    </figure>
		    <figure class="secondary_color yellow">
		      <div class="color"></div>
		      <h6 class="slate name">Yellow</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC">
		      </p>
		      <p class="desc rgb">
		        <input type="text" readonly="readonly" name="rgb" value="247 250 252">
		      </p>
		    </figure>
		  </div>
		  <!-- END SECONDARY -->
		  <!-- START GRADIENTS -->
			<h3 class="slate">Gradients</h3>
			<p class="gray">Each feature on the marketing site will have it’s own unique gradient used in the hero background. These will otherwise be used minimally.</p>
		  <div class="section gradient flex">
		    <figure class="gradient_color purple_grad">
		      <div class="color"></div>
		      <h6 class="slate name">Purple</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="454E5A" class="gradinput">
		        <span>to</span>
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		      </p>
		    </figure>
		    <figure class="gradient_color deep_blue_grad">
		      <div class="color"></div>
		      <h6 class="slate name">Deep Blue</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="45A0F8" class="gradinput">
		        <span>to</span>
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		      </p>
		    </figure>
		    <figure class="gradient_color green_grad">
		      <div class="color"></div>
		      <h6 class="slate name">Green</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="8D989E" class="gradinput">
		        <span>to</span>
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		      </p>
		    </figure>
		  </div>
		  <div class="section gradient flex">
		    <figure class="gradient_color mars_grad">
		      <div class="color"></div>
		      <h6 class="slate name">Mars</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		        <span>to</span>
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		      </p>
		    </figure>
		    <figure class="gradient_color orange_grad">
		      <div class="color"></div>
		      <h6 class="slate name">Orange</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		        <span>to</span>
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		      </p>
		    </figure>
		    <figure class="gradient_color cold_blue_grad">
		      <div class="color"></div>
		      <h6 class="slate name">Cold Blue</h6>
		      <p class="desc hex">
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		        <span>to</span>
		        <input type="text" readonly="readonly" name="hex" value="F7FAFC" class="gradinput">
		      </p>
		    </figure>
		  </div>
		  <!-- END GRADIENTS -->
		</section>
		<!-- END COLORS -->

		<!-- START TYPOGRAPHY -->
		<section id="typography" data-scroll-index="2">
			<!-- START HEADINGs -->
			<h3 class="heading slate">Headings</h3>
			<p class="gray">Use Brandon Grotesque for all heading and sub-headings. These headings should be uppercase in all instances.</p>
		  <div class="section headings">
		    <figure class="h1 slate">
		    	<h5 class="desc gray">h1 header / Brandon grotesque black, 46 PX, UPPERCASE</h5>
		      <h1 class="slate">Lorem ipsum dolor sit, consectetur adipiscing</h1>
		    </figure>
		    <figure class="h2 slate">
		    	<h5 class="desc gray">h2 header / Brandon grotesque black, 30 PX, UPPERCASE</h5>
		      <h2 class="slate">Lorem ipsum dolor sit amet, consectetur adipiscing elit DOLAR UNIMA SET</h2>
		    </figure>
		    <figure class="h3 slate">
		    	<h5 class="desc gray">h3 header / Brandon grotesque black, 24 PX, UPPERCASE</h5>
		      <h3 class="slate">Lorem ipsum dolor sit amet, consectetur adipiscing elit DOLAR UNIMA SET</h3>
		    </figure>
		    <figure class="h4 slate">
		    	<h5 class="desc gray">h4 header / Brandon grotesque black, 18 PX, UPPERCASE</h5>
		      <h4 class="slate">Lorem ipsum dolor sit amet, consectetur adipiscing elit DOLAR UNIMA SET</h4>
		    </figure>
		  </div>
		  <!-- END headings -->
		  <!-- START Paragraphs -->
			<h3 class="paragraph slate">Paragraphs</h3>
			<p class="gray">Use Source Sans Pro for all paragraph text and Ropa Sans for captions.</p>
		  <div class="section paragraphs">
		    <figure class="h1 slate">
		    	<h5 class="desc slate">PARAGRAPH TEXT / SOURCE SANS PRO, 18 PX</h5>
		      <p class="gray">Funnels are a way to visually measure how customers move through any series of events. Creating a funnel in Mixpanel is as simple as choosing an event.</p>
					<p class="gray">Other analytics tools ask you to set variables or rules, use fancy syntax, or fill in complicated forms. Just tell Mixpanel each event you want to track and you're done.</p>
		    </figure>
		  </div>
		  <!-- END Paragraphs -->
		</section>
		<!-- END COLORS -->
		<!-- <?php include('inc/right.php'); ?> -->
	</div>

<!-- END CONTENT -->
</div>